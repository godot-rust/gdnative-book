...
Once you have a Godot project, you can register GDExtension libraries that it should use. These libraries are loaded during startup of the Godot
engine -- both when opening the editor and when launching your game. The registration requires two files.
...
Every class inherits an existing Godot-provided class (its _base class_ or just _base_). You cannot inherit other user-defined classes. 
